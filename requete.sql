...
SELECT COUNT(contact.id) FROM contact, client, users WHERE contact.client_id = client.id AND contact.user_id = users.id AND YEAR(date) = 2020 AND client.entreprise_id = 42 GROUP BY contact.client_id
...
SELECT rdv.user_id, COUNT(rdv.user_id) FROM rdv, users WHERE rdv.user_id = users.id AND YEAR(date) = 2020 AND users.entreprise_id = 42 GROUP BY rdv.user_id